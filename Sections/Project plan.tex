...
Our project goals will be separated into three different categories; performance goals, result goals and learning goals. Performance goals are the targets that the group sets for themselves in the long-term, with the aim of achieving a desired level of performance or outcome. Result goals are focused on the specific result or outcomes that the group aims to achieve at the ending of the project. These goals outline what the final product will be and the value it will provide to the stakeholder. Learning goals are the knowledge we want to acquire during the project, and what new skills we want to be left with, and after the project ends - the acquired knowledge and skills are intended to be retained and used in the future. 
...
\begin{itemize}
    ...
    \item The group will be as efficient as possible during the workhours. Which are from 10-12. However, the reamining hours group members are allowed to work on different aspects of the thesis were it is mandatory for a bit in-depth work, but that is not crucial to have done at that very time. 
...
The main priority for testing of tools is to go through the most popular tools, so that it applies to majority of users. 
At this moment in time it seems that the stakeholder is not interested in setting up an environment for the group to do tests in as well as giving licenses for different tools that cost money. The group has therefore decided that the main focus will be on tools that are free and test these to the groups best ability. 

In addition the group will focus primarily on parts five (Product Testing and Integration) and six (Deployment and Maintenance Of Product) of the \acrshort{sdlc}, but if desired it can be necessary to look at step four (Developing Product). This is because it seems the most necessary to be exploring tools related to deployment pipelines, and thus will not focus on the earlier stages of the \acrshort{sdlc} since this is outside the scope and wishes of \acrshort{nbim}.
...
The group leader is responsible for making sure all members of the group is doing their part of the project, as well as making sure all members are cooperating well. If any issues would occur between members of the group, it is the group leaders responsibility to find a solution.
...
\\~\\
The head of communication will also be the main speaker during larger meetings with supervisor and external business, which also includes creating agendas for these meetings. The smaller meetings however, that last over a shorter period of time (5-15 minutes), will be a team effort were everyone can speak. 
...
The secretary will then add all reports to a folder created in Teams, were the head of documentation will make sure that it has been done correctly. 

Secretary is: Thea Urne 
\\~\\
\textbf{Head of sources}

The head of sources role will be divided between two people, were they will control that sources are written properly and that the group follow the right structure. 

The head of sources will be: Sebastian Hestsveen and Celina Heimdal Brynildsen
\\~\\
\textbf{Head of quality assurance }

The head of quality assurance will also be divided between two people, were the weekly will read over everything that's being written that week to make sure that all typos have been handled and that the structure of the text is the way the group want it to be. 
